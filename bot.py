...
TOKEN = os.getenv('8407865058:AAEL3taELI81wRo6Y3Q0fGFJ3jMCvTt2_Y8')
OWNER_ID_STR = os.getenv('8103866669')
ADMIN_ID_STR = os.getenv('8103866669')
YOUR_USERNAME = os.getenv('@Developer_Asanur')
UPDATE_CHANNEL = os.getenv('https://t.me/zx_community')
...
UPDATE_CHANNEL = UPDATE_CHANNEL or 'https://t.me/zx_community'
YOUTUBE_CHANNEL = 'https://www.youtube.com/@ASEDITZ-100k'
...
# ── FORCE-SUBSCRIBE (Telegram channel join mandatory) ──────────────
...
async def is_subscribed(user_id: int) -> bool:
    try:
        ...
        return True  # agar check fail ho (bot admin nahi hai channel mein, etc.) to block mat karo

def get_join_keyboard():
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="📢 Join Channel", url=UPDATE_CHANNEL)],
...
Bot use karne ke liye pehle hamara Telegram channel join karo 👇

Join karne ke baad <b>"✅ I've Joined"</b> button dabao."""

class ForceSubMiddleware(BaseMiddleware):
    async def __call__(self, handler, event, data):
        user = data.get("event_from_user")
        if user:
            is_join_callback = isinstance(event, types.CallbackQuery) and event.data == "check_join"
            if not is_join_callback and user.id != OWNER_ID and user.id != ADMIN_ID:
                if not await is_subscribed(user.id):
                    if isinstance(event, types.Message):
                        await event.answer(JOIN_LOCK_TEXT, reply_markup=get_join_keyboard(), parse_mode="HTML")
                    elif isinstance(event, types.CallbackQuery):
                        await event.answer("🔒 Pehle channel join karo!", show_alert=True)
                    return
...
async def callback_check_join(callback: types.CallbackQuery):
    if await is_subscribed(callback.from_user.id):
        ...
        welcome_text = build_welcome_text(callback.from_user.id, callback.from_user.full_name)
        await bot.send_message(callback.from_user.id, welcome_text, reply_markup=get_main_keyboard(callback.from_user.id), parse_mode="HTML")
    else:
        await callback.answer("❌ Abhi tak join nahi kiya! Pehle channel join karo.", show_alert=True)
